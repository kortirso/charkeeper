...
describe HomebrewsV2Context::Publications::PerformService do
  ...
  context 'for valid domain, feature with options' do
    let(:file_path) { Rails.root.join('spec/fixtures/daggerheart/domain_with_options.json') }
    let(:file) { Rack::Test::UploadedFile.new(file_path, 'application/json') }
    ...
    it 'calls import command', :aggregate_failures do
      expect { service_call }.to(
        change(Daggerheart::Homebrews::Domain, :count).by(1)
          .and(change(Daggerheart::Feat, :count).by(3))
      )
      expect(Daggerheart::Feat.where.not(options: nil).size).to eq 1
      expect(publication.reload.errors_list).to eq({})
    end
  end
