...
describe Frontend::Dnd2024::Characters::SpellsController do
  ...
  let!(:spell) { create :spell, :dnd2024 }

  describe 'GET#index' do
    context 'for logged users' do
      before { create :character_spell, spell: spell, character: user_character }
      ...
      context 'for user character' do
        it 'returns data', :aggregate_failures do
          ...
          expect(response_values.keys).to(
            contain_exactly('id', 'ready_to_use', 'prepared_by', 'spell_ability', 'notes', 'slug', 'name', 'level', 'spell_id')
          )
        end
      end
    end
      ...
      context 'for user character' do
        context 'for unexisting spell' do
          ...
          it 'does not create character spell', :aggregate_failures do
            expect { request }.not_to change(Dnd2024::Character::Spell, :count)
            expect(response).to have_http_status :not_found
          end
        end
  ...
  describe 'PATCH#update' do
    context 'for logged users' do
      ...
      context 'for user character' do
        ...
        context 'for existing spell' do
          let!(:character_spell) {
            create :character_spell, spell: spell, character: user_character, data: { ready_to_use: true }
          }
          let(:request) {
            patch :update, params: {
              character_id: user_character.id,
          ...
          it 'updates character spell', :aggregate_failures do
            ...
            expect(character_spell.reload.data['ready_to_use']).to be_falsy
            expect(response).to have_http_status :ok
