...
describe Frontend::Dc20::CharactersController do
  ...
  describe 'POST#create' do
    context 'for logged users' do
      let(:request) {
        post :create, params: {
          character: {
            name: 'Грундар', main_class: 'cleric', ancestry_feats: { human: ['feat'] }
      ...
      context 'for invalid request' do
        let(:request) {
          post :create, params: {
            character: {
              name: 'Грундар', main_class: 'zephyr', ancestry_feats: { human: ['feat'] }
            }, charkeeper_access_token: access_token
          }
        }
