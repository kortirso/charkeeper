...
describe HomebrewsV2::Daggerheart::SpecialitiesController do
  let!(:user_session) { create :user_session }
  let(:access_token) { Authkeeper::GenerateTokenService.new.call(user_session: user_session)[:result] }
  let!(:own_element) { create :homebrew, :daggerheart_class, user: user_session.user }
  let!(:element) { create :homebrew, :daggerheart_class }
    ...
    context 'for logged users' do
      ...
      context 'for existing homebrew' do
        ...
        it 'returns data', :aggregate_failures do
          ...
          expect(response.parsed_body['homebrew'].keys).to contain_exactly('id', 'features', 'domains', 'info')
        end
      end
    end
  end
    ...
    context 'for logged users' do
      ...
      context 'for existing homebrew with using for character' do
        ...
        before do
          ...
          character.data['subclasses'] = { own_element.id.to_s => nil }
          character.save
        end
    ...
    context 'for logged users' do
      ...
      context 'for existing own homebrew' do
        ...
        it 'returns error', :aggregate_failures do
          expect { request }.not_to change(Daggerheart::Homebrews::Speciality, :count)
          expect(response).to have_http_status :not_found
        end
      end
      ...
      context 'for existing homebrew' do
        ...
        it 'creates homebrew', :aggregate_failures do
          expect { request }.to change(Daggerheart::Homebrews::Speciality, :count).by(1)
