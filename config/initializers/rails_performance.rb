...
if defined?(RailsPerformance)
  RailsPerformance.setup do |config|
    ...
    config.http_basic_authentication_user_name = Rails.application.credentials.dig(:admin, :username)
    config.http_basic_authentication_password = Rails.application.credentials.dig(:admin, :password)

    # if you need an additional rules to check user permissions
    config.verify_access_proc = proc { |controller| true } # rubocop: disable Lint/UnusedBlockArgument
    ...
    config.ignored_paths = ['/performance', '/adminbook', '/solid_errors']
