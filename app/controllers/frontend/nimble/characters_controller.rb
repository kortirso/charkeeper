...
module Frontend
  module Nimble
    class CharactersController < Frontend::Nimble::BaseController
      include Deps[
        character_create: 'commands.characters_context.nimble.create',
      ...
      def render_character(result, fields, status)
        serialize_resource(result, ::Nimble::CharacterSerializer, :character, fields, status)
      end

      def character
        authorized_scope(Character.all).nimble.find(params.expect(:id))
      end
