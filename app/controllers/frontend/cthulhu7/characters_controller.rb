...
module Frontend
  module Cthulhu7
    class CharactersController < Frontend::BaseController
      include Deps[
        character_create: 'commands.characters_context.cthulhu7.create',
        character_update: 'commands.characters_context.cthulhu7.update'
      ]
      ...
      def render_character(result, fields, status)
        serialize_resource(result, ::Cthulhu7::CharacterSerializer, :character, fields, status)
      end

      def character
        authorized_scope(Character.all).cthulhu7.find(params.expect(:id))
      end
