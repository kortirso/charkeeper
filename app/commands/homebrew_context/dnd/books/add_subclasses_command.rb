...
module HomebrewContext
  module Dnd
    module Books
      class AddSubclassesCommand < BaseCommand
        use_contract do
          ...
          input[:attributes] = ids(input).map do |id|
            {
              ...
              itemable_type: 'Dnd2024::Homebrew::Subclass',
              itemable_id: id
            }
          end
        end
        ...
        def ids(input)
          existing_subclasses_ids = input[:book].items.where(itemable_type: 'Dnd2024::Homebrew::Subclass').pluck(:itemable_id)
          relation =
            Homebrew::Subclass
              .where(type: 'Dnd2024::Homebrew::Subclass', user_id: input[:user].id)
