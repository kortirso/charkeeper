      ...
      module Communities
        class PerformCommand < BaseCommand
          ...
          def validate_content(input) # rubocop: disable Metrics/AbcSize
            if input.key?(:id)
              input[:community] = ::Daggerheart::Homebrews::Community.find_by(user_id: input[:user].id, id: input[:id])
              return ['Not found'] unless input[:community]
            end

            input[:features] = input[:features]&.map!(&:deep_symbolize_keys)
            input[:features]&.each do |feature|
              feature[:user] = input[:user]
              feature[:origin] = 'community'
              feature[:origin_value] = 'community.id'
          ...
          def do_persist(input)
            command =
              if input[:community]
                HomebrewsV2Context::Import::Daggerheart::Communities::ChangeCommand.new
              else
                HomebrewsV2Context::Import::Daggerheart::Communities::AddCommand.new
              end
            command.call(input)
          end
