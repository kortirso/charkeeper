        ...
        class ChangeCommand < BaseCommand
          ...
          def do_persist(input)
            ActiveRecord::Base.transaction do
              input[:transformation].update!(input.slice(:title, :description, :public))
          ...
          def change_features(input)
            input[:features].each do |feature|
              if feature[:id]
                ...
                change_feat.call(
                  feature.except(:id).merge({ feat: existing_feature, origin_value: input[:transformation].id, no_refresh: true })
                )
              else
                add_feat.call(
                  feature.merge({ origin_value: input[:transformation].id, no_refresh: true })
                )
              end
            end
          end
