        ...
        class AddCommand < BaseCommand
          ...
          def do_persist(input)
            result = ActiveRecord::Base.transaction do
              transformation = ::Daggerheart::Homebrews::Transformation.create!(input.slice(:user, :title, :description, :public))
              input[:features].each do |feature|
                add_feat.call(
                  feature.merge({
                    user: input[:user], origin: 'transformation', origin_value: transformation.id, no_refresh: true
                  })
                )
              end
              transformation
