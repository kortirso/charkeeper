      ...
      module Transformations
        class PerformCommand < BaseCommand
          ...
          def validate_content(input) # rubocop: disable Metrics/AbcSize
            if input.key?(:id)
              input[:transformation] = ::Daggerheart::Homebrews::Transformation.find_by(user_id: input[:user].id, id: input[:id])
              return ['Not found'] unless input[:transformation]
            end
            ...
            input[:features]&.each do |feature|
              ...
              feature[:origin_value] = 'transformation.id'
          ...
          def do_persist(input)
            command =
              if input[:transformation]
                HomebrewsV2Context::Import::Daggerheart::Transformations::ChangeCommand.new
              else
                HomebrewsV2Context::Import::Daggerheart::Transformations::AddCommand.new
              end
            command.call(input)
          end
