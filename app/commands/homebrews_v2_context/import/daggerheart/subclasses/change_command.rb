          ...
          def change_features(input)
            input[:features].each do |feature|
              if feature[:id]
                ...
                change_feat.call(
                  feature.except(:id).merge({ feat: existing_feature, origin_value: input[:subclass].id, no_refresh: true })
                )
              else
                add_feat.call(
                  feature.merge({ origin_value: input[:subclass].id, no_refresh: true })
                )
              end
            end
          end
