        ...
        class PerformCommand < BaseCommand
          ...
          use_contract do
            ...
            params do
              ...
              required(:items).filled(:array).each(:hash) do
                optional(:id).filled(:string, :uuid_v4?)
                required(:title).hash do
                  required(:en).filled(:string, max_size?: 50)
                  optional(:ru).maybe(:string, max_size?: 50)
                ...
                optional(:features).maybe(:array).each(:hash) do
                  optional(:id).filled(:string, :uuid_v4?)
                  required(:title).hash do
                    optional(:en).maybe(:string, max_size?: 50)
                    optional(:ru).maybe(:string, max_size?: 50)
                    optional(:es).maybe(:string, max_size?: 50)
                  end
                  required(:description).hash do
                    optional(:en).maybe(:string, max_size?: 1_000)
          ...
          def validate_content(input)
            ...
            input[:mechanic] = ::Daggerheart::Homebrews::Mechanic.find_by(user_id: input[:user].id, id: input[:id])
            return if input[:mechanic]
          ...
          def do_persist(input)
            command =
              if input[:mechanic]
                HomebrewsV2Context::Import::Daggerheart::Mechanics::ChangeCommand.new
              else
                HomebrewsV2Context::Import::Daggerheart::Mechanics::AddCommand.new
              end
            command.call(input)
          end
        end
