      ...
      module Ancestries
        class ChangeCommand < BaseCommand
          include Deps[
            ...
            change_feat: 'commands.homebrews_v2_context.import.nimble.feats.change',
          ...
          def do_persist(input)
            ActiveRecord::Base.transaction do
              input[:ancestry].update!(input.slice(:title, :description, :public, :info))
          ...
          def change_features(input)
            input[:features].each do |feature|
              if feature[:id]
                ...
                change_feat.call(
                  feature.except(:id).merge({ feat: existing_feature })
                )
              else
                add_feat.call(
                  feature.merge({ origin_value: input[:ancestry].id })
                )
              end
            end
          end
