        ...
        class ChangeCommand < BaseCommand
          ...
          def do_prepare(input)
            input[:existing_features] =
              ::Cosmere::Feat
                .where(origin: 'specialization', origin_value: input[:specialization].id)
                .group_by(&:id)
          ...
          def do_persist(input)
            ActiveRecord::Base.transaction do
              input[:specialization].update!(input.slice(:title, :description, :public, :info))
            ...
            cache.push_item(key: :specializations, item: input[:specialization])
          ...
          def change_features(input)
            input[:features].each do |feature|
              if feature[:id]
                ...
                change_feat.call(
                  feature.except(:id).merge({ feat: existing_feature, origin_value: input[:specialization].id })
                )
              else
                add_feat.call(
                  feature.merge({ origin_value: input[:specialization].id })
                )
              end
            end
          end
