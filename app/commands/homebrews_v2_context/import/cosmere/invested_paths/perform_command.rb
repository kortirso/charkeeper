      ...
      module InvestedPaths
        class PerformCommand < BaseCommand
          ...
          def validate_content(input) # rubocop: disable Metrics/AbcSize
            if input.key?(:id)
              input[:invested_path] = ::Cosmere::Homebrews::InvestedPath.find_by(user_id: input[:user].id, id: input[:id])
              return ['Not found'] unless input[:invested_path]
            end

            input[:features] = input[:features]&.map!(&:deep_symbolize_keys)
            input[:features]&.each do |feature|
              feature[:user] = input[:user]
              feature[:origin] = 'radiant_path'
              feature[:origin_value] = 'invested_path.id'
          ...
          def do_persist(input)
            command =
              if input[:invested_path]
                HomebrewsV2Context::Import::Cosmere::InvestedPaths::ChangeCommand.new
              else
                HomebrewsV2Context::Import::Cosmere::InvestedPaths::AddCommand.new
              end
            command.call(input)
          end
