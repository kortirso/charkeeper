  ...
  module Representers
    class Roll
      ...
      def represent_web(command_result)
        return { errors: [I18n.t('services.bot_context.representers.roll.nothing')] } if command_result[:rolls].empty?

        formatted_result = command_result[:rolls].map { |item| "#{item[0]} (#{item[1]})" }.join(', ')
        {
          result: "#{I18n.t('services.bot_context.representers.roll.result')}: #{formatted_result}<br />#{I18n.t('services.bot_context.representers.roll.total')}: #{command_result[:total]}"
        }
      end
      ...
      def represent_telegram(command_result)
        ...
        formatted_result = command_result[:rolls].map { |item| "#{item[0]} (#{item[1]})" }.join(', ')
        {
          result: "<b>#{I18n.t('services.bot_context.representers.roll.result')}</b>: #{formatted_result}\n<b>#{I18n.t('services.bot_context.representers.roll.total')}</b>: #{command_result[:total]}"
        }
      end
