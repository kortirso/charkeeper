...
module SheetsContext
  class Generate
    ...
    def daggerheart_pdf(character)
      pdf = CombinePDF.load Rails.root.join('app/services/sheets_context/daggerheart/template.pdf')

      pdf_data =
        SheetsContext::Daggerheart::Template.new(
    ...
    def dnd5_pdf(character)
      pdf = CombinePDF.load Rails.root.join('app/services/sheets_context/dnd/template.pdf')

      pdf_data =
        SheetsContext::Dnd5::Template.new(
          page_size: 'A4', page_layout: :portrait, margin: 0
        ).to_pdf(character: character.decorator)
    ...
    def dnd2024_pdf(character)
      ...
      pdf_data =
        SheetsContext::Dnd2024::Template.new(
          page_size: 'A4', page_layout: :portrait, margin: 0
        ).to_pdf(character: character.decorator)
    ...
    def pathfinder2_pdf(character)
      pdf = CombinePDF.load Rails.root.join('app/services/sheets_context/pathfinder2/template.pdf')

      pdf_data =
        SheetsContext::Pathfinder2::Template.new(
          page_size: 'A4', page_layout: :portrait, margin: 0
        ).to_pdf(character: character.decorator)
