class BackfillDataForDhHomebrews < ActiveRecord::Migration[8.1]
  def up
    Homebrew::Book::Item.where(itemable_type: 'Homebrew').each do |item|
    ...
    Homebrew::Book::Item.where(itemable_type: 'Homebrew').each do |item|
      new_item = ::Daggerheart::Homebrews::Domain.find_by(id: item.itemable_id)
      next unless new_item

      item.update(itemable_id: new_item.id, itemable_type: 'Daggerheart::Homebrews::Domain')
    end
  end
